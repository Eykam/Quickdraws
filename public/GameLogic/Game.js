...
let walk = () => {
  ...
  currCoordRight = currCoordRight + walkDistance;
  rightPlayerCharacter.style.left = currCoordRight + "%";

  leftPlayerCharacter.src = imgLeft;
  currCoordLeft = currCoordLeft - walkDistance;
...
function walking() {
  ...
  walkInterval = setInterval(walk, walkSpeed);
}
...
function setShot(direction) {
  if (direction == "right") rightPlayerCharacter.src = baseDirRight + baseShoot;
  else leftPlayerCharacter.src = baseDirLeft + baseShoot;
}
